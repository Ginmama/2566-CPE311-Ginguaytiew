...
void Play_Sound_200ms(void);

uint8_t flag = 0; //use for Toggle between 0 and 1 for switching notes
...
void TIM_BASE_Config(uint16_t note) {
...
void Play_Sound_200ms(void) {
    ...
    TIM_OC_Config(note_CALCULATE(E_O6));
    //LL_mDelay(200); // Delay for 200 ms
    //LL_TIM_DisableCounter(TIM4); // Stop sound
}
